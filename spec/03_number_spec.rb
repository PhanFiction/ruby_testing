...
describe Array do
  context 'when my_array has perfect values' do
    ...
    subject(:my_array) { (1..10).to_a }

    # remove the 'x' before running this test
    it 'has a specific first value' do
      expect(my_array.first).to be_odd.and be < 2
    end

    # remove the 'x' before running this test
    it 'has a specific last value' do
      expect(my_array.last).to be > 9
    end

    # remove the 'x' before running this test
    it 'has a specific min value' do
      expect(my_array.min).not_to be < -1
    end

    # remove the 'x' before running this test
    it 'has a specific max value' do
      expect(my_array.max).not_to be > 10
    end

    # remove the 'x' before running this test
    it 'includes a value of 5' do
      expect(my_array).to include(5)
    end

    # remove the 'x' before running this test
    it 'has a fourth element' do
      expect(my_array[3]).not_to be nil
    end
  end
end
