...
describe String do
  ...
  context 'when favorite food is updated' do
    ...
    it 'updates the favorite food' do
      ...
      expect(favorite_food).to be('apple')
    end
  end

  # nested context
  context 'when favorite food is updated' do
    let(:favorite_food) { String.new('orange') }
    context('When the food is not the same') do
      it 'food orange is not the same as apple' do
        expect(favorite_food).not_to be('apple')
      end
    end
  end
end
